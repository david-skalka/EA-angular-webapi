    ...
    (InitializeOptions options) =>
{
    ...
    var pipeline = new IGeneratorCommand[] {
        ...
        new ShellGeneratorCommand("dotnet", "new nunit -f net8.0 -n " + options.ProjectName + "IntegrationTest -o \"" + Path.Combine( outputDir, options.ProjectName + "IntegrationTest"), null),
        new MkdirGeneratorCommand(Path.Combine(outputDir, options.ProjectName+"IntegrationTest", "Seeders")),
        new ShellGeneratorCommand("dotnet", "add package Microsoft.AspNetCore.Mvc.Testing --version 8.0.6", Path.Combine(outputDir, options.ProjectName+ "IntegrationTest")),
        new ShellGeneratorCommand("dotnet", "add reference ../" + options.ProjectName, Path.Combine(outputDir, options.ProjectName+ "IntegrationTest")),
        new ShellGeneratorCommand("dotnet", "dotnet sln " + options.ProjectName + ".sln add "+options.ProjectName+" "+options.ProjectName+"IntegrationTest", outputDir),
        new RmGeneratorCommand(Path.Combine(outputDir, options.ProjectName, "Program.cs")),
        new WriteCallbackResultGeneratorCommand(() => new EADotnetWebapiCli.Templates.Program(){ ProjectName = options.ProjectName }.TransformText(), Path.Combine(outputDir,  options.ProjectName, "Program.cs")),
        new RmGeneratorCommand(Path.Combine(outputDir, options.ProjectName+"IntegrationTest", "UnitTest1.cs")),
        new MkdirGeneratorCommand(Path.Combine(outputDir, options.ProjectName, options.ProjectName + "IntegrationTest", "Seeders")),
        new WriteCallbackResultGeneratorCommand(() => new ISeeder(){ ProjectName = options.ProjectName }.TransformText(), Path.Combine(outputDir,  options.ProjectName+ "IntegrationTest", "ISeeder.cs")),
        new WriteCallbackResultGeneratorCommand(() => new CustomWebApplicationFactory(){ ProjectName = options.ProjectName }.TransformText(), Path.Combine(outputDir,  options.ProjectName+ "IntegrationTest", "CustomWebApplicationFactory.cs")),
    };
    Generate(pipeline);
    return 0;
},
